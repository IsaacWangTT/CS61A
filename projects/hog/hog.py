...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    my_score = 0
    if num_rolls == 0:
        my_score += piggy_points(opponent_score)
    else:
...
def more_boar(player_score, opponent_score):
    ...
    if len(str(player_score)) == 1:
        ps = "0" + str(player_score)
    else:
        ps = str(player_score)
    if len(str(opponent_score)) == 1:
        os = "0" + str(opponent_score)
    else:
        os = str(opponent_score)
    ...
    def say(score0, score1):
        ...
        if temp > running_high:
            ...
            print(
                "Player",
                who,
                "has reached a new maximum point gain.",
                temp_h,
                "point(s)!",
            )
        return announce_highest(who, score, temp_h)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1, 11):
        cur_sco = make_averaged(roll_dice, trials_count)(i, dice)
        if cur_sco > max_sco:
            max_num = i
            max_sco = cur_sco
    return max_num
